...
########
## Disentangling correlation from causation in the putatitve relationship between significantly genetically correlated traits and FEV1
## 2020 - William Reay
########
...
option_list = list(
  make_option("--phenotype_name", action="store", default=NA, type='character',
              help="The name of the trait to be analysed with FEV1 [required]"),
...
#Load in munged summary statistics for FEV1 and trait to be analysed

cat("#########################")
cat("\n")
cat("Loading summary statistics for FEV1 and", paste(opt$phenotype_name))
cat("\n")
cat("#########################")
cat("\n")
...
#Load trait 2 data 
FEV1_df <- read.table("../LDSC/LD_hub/Munged_FEV1_meta.sumstats" ,header=TRUE,sep="\t",stringsAsFactors = FALSE, na.strings=c("", "NA"))
FEV1_df <- na.omit(FEV1_df)
...
Annotated_df_combined <- merge(Merged_df,FEV1_df,by="SNP")
...
cat("#########################")
cat("\n")
cat("Constructing LCV model, output will be sent to text file",  paste(opt$phenotype_name, '_LCV_FEV1.txt', sep = ""))
cat("\n")
cat("#########################")
cat("\n")
...
file = paste(opt$phenotype_name, '_LCV_FEV1.txt', sep = "")
