...
########
## Disentangling correlation from causation in the putatitve relationship between significantly genetically correlated traits and FVC
## 2020 - William Reay
########
...
option_list = list(
  make_option("--phenotype_name", action="store", default=NA, type='character',
              help="The name of the trait to be analysed with FVC [required]"),
...
cat("#########################")
cat("\n")
cat("Loading summary statistics for FVC and", paste(opt$phenotype_name))
cat("\n")
cat("#########################")
cat("\n")
...
#Load trait 2 data 
FVC_df <- read.table("../LDSC/LD_hub/Munged_FVC_meta.sumstats" ,header=TRUE,sep="\t",stringsAsFactors = FALSE, na.strings=c("", "NA"))
...
file = paste(opt$phenotype_name, '_LCV_FVC.txt', sep = "")
