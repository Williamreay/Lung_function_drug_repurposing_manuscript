...
## Define linear model to test the effect of FVC PGS (P < 1) on these genes
...
write.csv(PGS_all_SNPs_FINAL, file = "Genome_wide_PGS_results/FVC_PGS_all_SNPs.csv")

########################
## Genome wide PGS - P < 0.05
#########################

## Define linear model to test the effect of FVC PGS (P < 0.05) on these genes
...
write.csv(PGS_0_05_threshold_FINAL, file = "Genome_wide_PGS_results/FVC_PGS_0_05_threshold.csv")

########################
## Genome wide PGS - P < 0.005
#########################

## Define linear model to test the effect of FVC PGS (P < 0.005) on these genes

PGS_0_005_threshold_model <- function(v){
  PGS_0_005_threshold_regresssion_result <- lm(glue::glue('{v} ~ SEX + PC1 + PC2 + PC3 + Genome_wide_PGS_0_005_threshold'), data = Merged_genotype_mRNA_geuvadis)
  return(summary(PGS_0_005_threshold_regresssion_result)) 
}

## Iterate function over list of genes

PGS_0_005_threshold <- sapply(List_available_ID, PGS_0_005_threshold_model)

## Extract t-statistic and p value for each gene

PGS_Extract_0_005_threshold <- apply(PGS_0_005_threshold, 2, function(x) return(as.data.frame(x$coefficients)[6,3:4]))

PGS_Results_0_005_threshold <- data.frame()
for (i in 1:length(PGS_Extract_0_005_threshold)) {
  PGS_Results_0_005_threshold <- rbind(PGS_Results_0_005_threshold, PGS_Extract_0_005_threshold[[i]])
}
rownames(PGS_Results_0_005_threshold) <- List_available_ID

## Apply FDR correction and export results

PGS_0_005_threshold_FDR <- p.adjust(PGS_Results_0_005_threshold$'Pr(>|t|)', method = "fdr")
PGS_0_005_threshold_FINAL <- cbind(PGS_Results_0_005_threshold, PGS_0_005_threshold_FDR)
write.csv(PGS_0_005_threshold_FINAL, file = "Genome_wide_PGS_results/FVC_PGS_0_005_threshold.csv")
